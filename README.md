...
- Uses a terminal-native layout: normal xterm input goes to the real shell, while input echo and output are shown through a default readable terminal mask. Persian/Arabic lines render as real RTL: right-aligned, `dir=rtl`, with English commands, paths, flags, and code kept as isolated LTR islands. You can toggle back to raw xterm with `Readable: off` for edge-case TUIs.
