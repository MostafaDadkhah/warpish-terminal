...
try {
  ...
  const terminalNativeUiVerified = !indexHtml.includes('terminal-input-mask')
    ...
    && appJs.includes('refreshReadableFromTmuxSoon(350, true)')
    && appJs.includes('refreshReadableFromTmuxSoon(200, true)')
    && appJs.includes('refreshBidiReaderFromCapture({ preferCapture })')
    && appJs.includes('const BIDI_READER_MAX_LINES = 2000')
    && appJs.includes('capture?lines=5000&ansi=1')
    && appJs.includes('refreshBidiReaderFromCapture({ keepScroll: true, preferCapture: true })')
    && appJs.includes('function scheduleBidiReaderCaptureAfterOutput()')
    && appJs.includes('BIDI_CAPTURE_SETTLE_DELAY_MS')
    && appJs.includes('term.write(new Uint8Array(event.data), handleTerminalWriteComplete)')
    && appJs.includes('const needsTmuxHistory = event.deltaY !== 0')
    && !appJs.includes('event.deltaY < 0 && (term.buffer?.active?.baseY ?? 0) === 0')
    ...
    && appJs.includes('XTERM_COLOR_MODE_P256')
    && appJs.includes('mode === XTERM_COLOR_MODE_PALETTE || mode === XTERM_COLOR_MODE_P256')
    && !appJs.includes('mode === XTERM_COLOR_MODE_RGB || mode === 0x2000000')
    ...
    && appJs.includes('const xtermHasText = entriesHaveVisibleText(entries)')
    && appJs.includes('const shouldUseCapture = (isTerminalAlternateBuffer() && (!xtermHasText || xtermIsSparse))')
    && appJs.includes('isBidiReaderHistoryMode() && lastCapturedReaderEntries.length > entries.length')
    && !appJs.includes('isSparseReadableEntries(entries) || lastCapturedReaderEntries.length > 0')
    ...
    && stylesCss.includes('cursor: pointer')
    && stylesCss.includes('text-decoration: underline')
