...
- the readable terminal mask is on by default, covers the terminal surface, re-renders recent xterm/tmux-captured lines as normal HTML, splits an LTR shell prompt from a following RTL input suffix, renders that suffix as a compact Word-style `dir=rtl` segment, wraps English commands, paths, flags, and code as isolated `dir=ltr` islands, preserves live xterm ANSI/palette/truecolor foreground/background/bold styling in the readable mask, dims shell suggestion text after the cursor, throttles/cache-skips fast redraws, and uses tmux capture for wheel scroll when xterm scrollback is unavailable;
