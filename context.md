...
- the readable terminal mask is on by default, covers the terminal surface, re-renders recent xterm/tmux-captured lines as normal HTML, splits an LTR shell prompt from a following RTL input suffix, renders that suffix as a compact Word-style `dir=rtl` segment, wraps English commands, paths, flags, and code as isolated `dir=ltr` islands, dims shell suggestion text after the cursor, throttles/cache-skips fast redraws, and uses tmux capture for wheel scroll when xterm scrollback is unavailable;
...
- Browser QA verified readable terminal interaction fixes: old live session reattach returns focus to `xterm-helper-textarea` and real keyboard input reaches the PTY; wheel over the readable surface expands from xterm's 23 visible rows to tmux-captured history (149/360 lines in QA) without sending `^[[A`/history input; synthetic inline suggestion text after the cursor renders as `.bidi-ghost` opacity `0.46`; fast 320-line streaming output rendered only 3 readable redraws with the underlying xterm screen opacity at `0`, reducing flicker.
